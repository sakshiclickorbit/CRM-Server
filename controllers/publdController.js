...
exports.updatePublisher = async (req, res) => {
  ...
  try {
    ...
    // ✅ Step 2: If publisher is transferred, update ALL old adv_data
    if (String(previous_user_id) !== String(user_id)) {
      // Get username of the NEW assigned user
      ...
      if (!loginRows.length) {
        throw new Error(`Login user not found for user_id: ${user_id}`);
      }

      const newPublisherUsername = loginRows[0].username;
   ...
   WHERE pub_id = ?`,
      ...
      console.log(
        `✅ Historical adv_data transferred:
        pub_id=${pub_id},
        new user_id=${user_id},

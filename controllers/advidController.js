...
exports.createAdvertisement = async (req, res) => {
    try {
        ...
        const { adv_name, adv_id, user_id, geo, note, poc_email, acc_email, assign_user, assign_id, level, 
            vector } = req.body;

        // ✅ Validation: Ensure mandatory fields are provided
        if (!adv_name || !adv_id || !user_id) {
            return res.status(400).json({ success: false, message: "adv_name, adv_id, and user_id are required." });
        }
...
exports.getAdvertisementsByUserId = async (req, res) => {
  try {
    ...
    for (const row of ads) {
      const { bd_id, legal_name, billing_address, tax_type, tax_id, ...advFields } = row;
...
exports.getAllAdvertiserss = async (req, res) => {
  try {
    ...
    console.log("🟢 Fetching advertisers for user:", user_id);
...
exports.updateAdvertisement = async (req, res) => {
    ...
    try {
        ...
        await connection.query(
            ...
            [adv_name, geo, note, target, poc_email, acc_email, assign_user, assign_id, user_id, level, 
                vector , adv_id]
        );

        // ✅ Step 2: Update adv_data table
        await connection.query(
            `UPDATE adv_data SET user_id = ? WHERE adv_id = ?`,
            [user_id, adv_id]
        );
        ...
        if (previous_user_id !== user_id) {
         ...
         if (billing_details.length > 0) {
          ...
          const incomingIds = billing_details
              .filter((b) => b.id)
              .map((b) => b.id);
              ...
              if (b.id) {
                  ...
                  await connection.query(
                      `UPDATE advertiser_billing_details 
                       ...
                       WHERE id = ? AND adv_id = ?`,
                      [b.legal_name, b.billing_address, b.tax_type, b.tax_id, b.id, adv_id]
                  );
              } else {
                  // New row → INSERT
                  await connection.query(
                      `INSERT INTO advertiser_billing_details 
                       ...
                       VALUES (?, ?, ?, ?, ?, ?)`,
                      [adv_id, b.legal_name, b.billing_address, b.tax_type, b.tax_id, user_id]
                  );
              }
          }
      } else {

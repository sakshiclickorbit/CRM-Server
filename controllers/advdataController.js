...
exports.updateNote = async (req, res) => {
  try {
    ...
    if (result.affectedRows === 0) {
      return res.status(404).json({ success: false, message: 'Record not found or nothing changed.' });
    }

    // Optional: return updated row
    const [rows] = await db.query('SELECT * FROM adv_data WHERE id = ?', [id]);
    return res.json({ success: true, message: 'Record updated', data: rows[0] });
...
exports.updateAdvData = async (req, res) => {
  try {
    ...
    const allowedFields = [
      "pub_name", "campaign_name", "geo", "city", "os", "payable_event",
      "mmp_tracker", "adv_id", "adv_payout", "pub_id", "pid", "shared_date",
      "paused_date", "adv_total_no", "adv_deductions", "adv_approved_no",
      "user_id", "pay_out", "pub_Apno", "fa", "fa1", "vertical", "campaign_id"
    ];
...
console.log(rows,"rows")
    for (const row of rows) {
      ...
      if (
        ...
        console.log(`Skipping ID ${id} due to missing fields.${  adv_deductions,
            adv_approved_no,
            adv_payout,
            pay_out}`);
        continue;
      ...
      try {
        ...
        // Update the row
        await db.execute(
          "UPDATE adv_data SET pub_Apno = ? WHERE id = ?",
          [pub_Apno, id]
        );
